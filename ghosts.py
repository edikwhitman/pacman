...
class Ghost(AnimatedCharacter):
    ...
    def set_chase_mode(self, map, pacman_position):
        pos = self.get_ghost_cell()
        minimum_distance = 1000
        direction = 0
        if map[pos[1]][pos[0]-1] != '0' and self.get_points_distance((pacman_position[1], pacman_position[0]), (pos[1], pos[0] - 1)) <= minimum_distance and self.movement_direction != 4 and self.current_cell != pos:
            direction = 3
            minimum_distance = self.get_points_distance((pacman_position[1], pacman_position[0]), (pos[1], pos[0] - 1))

        if map[pos[1]][pos[0]+1] != '0' and self.get_points_distance((pacman_position[1], pacman_position[0]), (pos[1], pos[0] + 1)) <= minimum_distance and self.movement_direction != 3 and self.current_cell != pos:
            direction = 4
            minimum_distance = self.get_points_distance((pacman_position[1], pacman_position[0]), (pos[1], pos[0] + 1))

        if map[pos[1]+1][pos[0]] != '0' and self.get_points_distance((pacman_position[1], pacman_position[0]), (pos[1] + 1, pos[0])) <= minimum_distance and self.movement_direction != 1 and self.current_cell != pos:
            direction = 2
            minimum_distance = self.get_points_distance((pacman_position[1], pacman_position[0]), (pos[1] + 1, pos[0]))

        if map[pos[1]-1][pos[0]] != '0' and self.get_points_distance((pacman_position[1], pacman_position[0]), (pos[1] - 1, pos[0])) <= minimum_distance and self.movement_direction != 2 and self.current_cell != pos:
            direction = 1
            minimum_distance = self.get_points_distance((pacman_position[1], pacman_position[0]), (pos[1] - 1, pos[0]))
        if direction > 0:
